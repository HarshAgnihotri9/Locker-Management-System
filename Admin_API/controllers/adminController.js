...
import requestModel from "../models/userRequest.js";
...
const signup = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "SignUp Successfully", error: false, user: result });
  } catch (error) {
    console.log(error);
...
const GenerateRequest = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ message: "Your request already have been generated" });
    }
    ...
    res
      .status(201)
      .json({ user: result, message: "Request sent successfully" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Something went wrong" });
  }
...
const adminController = { signin, signup, GenerateRequest };
